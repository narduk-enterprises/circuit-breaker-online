...
```bash
# Hub-spoke: reference hub secrets (do not copy values)
doppler secrets set CLOUDFLARE_API_TOKEN='${narduk-nuxt-template.prd.CLOUDFLARE_API_TOKEN}' --project my-app --config prd
doppler secrets set GSC_SERVICE_ACCOUNT_JSON='${narduk-nuxt-template.prd.GSC_SERVICE_ACCOUNT_JSON}' --project my-app --config dev
```
...
| `GSC_SERVICE_ACCOUNT_JSON` | `← narduk-nuxt-template` hub ref               | `prd`  | Service account key (JSON or base64) for GSC API |
