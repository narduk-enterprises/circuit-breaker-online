...
```bash
git clone https://github.com/loganrenz/circuit-breaker-online.git my-project
cd my-project
pnpm install
...
| ---------------------- | ---------- | -------------- |
| `circuit-breaker-online` | `dev`      | Shared secrets |
| `circuit-breaker-online` | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project circuit-breaker-online --config dev_auth --no-interactive
```
