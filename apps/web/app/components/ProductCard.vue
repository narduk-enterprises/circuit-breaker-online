...
<template>
  <NuxtLink :to="`/products/${product.slug}`" class="product-card light-card group block rounded-xl overflow-hidden">
    <!-- Image -->
    ...
    <div class="p-3 sm:p-4">
      ...
      <!-- Name -->
      <h3 class="mb-1 line-clamp-2 font-display text-sm font-semibold text-default transition-colors group-hover:text-brand-600">
        {{ product.name }}
      </h3>
      ...
      <div v-if="product.voltage || product.amperage" class="mb-3 flex flex-wrap gap-1">
        <span v-if="product.voltage" class="inline-flex items-center rounded-full bg-brand-50 px-2 py-0.5 text-[10px] font-medium text-brand-700">
          ⚡ {{ product.voltage }}
        </span>
        <span v-if="product.amperage" class="inline-flex items-center rounded-full bg-brand-50 px-2 py-0.5 text-[10px] font-medium text-brand-700">
          🔌 {{ product.amperage }}
        </span>
      </div>
