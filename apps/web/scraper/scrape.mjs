...
const SITEMAPS = [
  `${BASE_URL}/product-sitemap1.xml`,
  `${BASE_URL}/product-sitemap2.xml`,
];
...
function parseProductPage(html, url) {
  ...
  // Description
  const description = $('.woocommerce-product-details__short-description').html()?.trim() ||
  ...
  if (breadcrumbs.length > 1) {
    category = breadcrumbs.eq(breadcrumbs.length - 1).text().trim();
    if (category.toLowerCase() === name.toLowerCase()) {
      category = breadcrumbs.eq(breadcrumbs.length - 2).text().trim();
    }
  }
  ...
  $('table.woocommerce-product-attributes tr, table.shop_attributes tr').each((_, row) => {
    const label = $(row).find('th, .woocommerce-product-attributes-item__label').text().trim().toLowerCase();
...
async function main() {
  ...
  for (const p of progress.products) {
    if (!categoryMap[p.category]) {
      categoryMap[p.category] = { name: p.category, slug: p.category.toLowerCase().replace(/[^a-z0-9]+/g, '-'), count: 0 };
    }
